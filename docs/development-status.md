...
**2026-07-01 — SQL Server 單庫 schema-per-project 遷移 + 單庫加固／Express 退場／四層子資料夾化**。SQL Server provider 由「每專案一個資料庫 `JET_{projectId}`」遷移為**單一資料庫 `JET` + 每專案一個 `prj_xxx` schema**(schema 名 = `prj_` + 淨化案名 + 8 位 SHA256)，連線改 **SQL 驗證**(jetapp／單庫，收斂單一 `appsettings.json`，公司電腦只改 `Sql:Server`)，並修啟動健康檢查 UI 死鎖與跨伺服器刪／建案的 master 前檢、新增 `system.databaseInfo`。此遷移已 **commit(`dd7b00d`)並經使用者實機驗收**。其後同日數個加固子輪(逐條見 `development-log.md` r2–r5)**已落地但尚未 commit**：① 單庫資料隔離雙守衛(`SchemaIsolationGuardTests` 靜態掃裸表名 + `SchemaIsolationJourneyTests` 雙專案真實引擎)；② 依賴方向機器守衛 `LayerDependencyTests`(NetArchTest 掃 IL；抓到並修復 9 個既有層違規——8 個分頁介面錯置 Infrastructure 已搬回 Domain、`system.databaseInfo` 改注入 `ISqlServerBackendProbe` port)；③ **SQL Server Express 退場**(EngineEdition=4 含 LocalDB，首次碰庫即以 `sql_server_express_unsupported` 硬擋；開發／測試環境收緊為「非 Express 且 ≥ 2022」)；④ 跨 provider 姓名排序 parity 修復(`COLLATE Latin1_General_BIN2`)；⑤ 測試連線改由 appsettings 驅動、測試庫收斂 `JET_Test`(jetapp 擁有)；⑥ **四層子資料夾化**(Infrastructure 131 檔 + Application 44 檔 + Domain 54 檔依 §14 分組，**只搬檔、namespace 不變**；Bridge 使用者裁定不拆)。全套件本機連 **SQL Server 2022 為 1113 綠 ／ 0 略過**(r3–r5 一致)；四層子資料夾化為純搬檔、`dotnet build` 0 警告 0 錯誤。**待連 2022 全套件回歸複驗(計畫 Task 11)、GUI 目視(本輪僅 Express 提示文案為使用者可見變更)與版控提交。**
...
**Provider 策略**:SQL 方言相異片段集中於 `ISqlDialect`(含 keyset 分頁的 `LimitClause`:SQLite `LIMIT`、SQL Server `OFFSET 0 ROWS FETCH NEXT`),規則述詞單一事實來源 `GlRulePredicates` 與 WHERE 組譯 `GlFilterWhereBuilder` 皆 provider 中立(DbCommand)。keyset 游標述詞一律展開布林式(`key > @cursor`),不用元組比較(SQL Server T-SQL 不支援),兩 provider 共用。**SQLite 與 SQL Server 皆已實作**(2026-06-14 完成全 repo 移植與 provider 路由,雙 provider 端到端等價)。**SQL Server 隔離模型(2026-07-01)**:單一資料庫 `JET` + 每專案一個 `prj_xxx` schema(不再是每專案一個庫);schema 前綴是唯一的跨專案隔離牆,由 `SchemaIsolationGuardTests`(靜態掃裸表名)與 `SchemaIsolationJourneyTests`(雙專案真實引擎)雙守衛。**目標引擎為 SQL Server 2022**(開發用 Developer;生產 Standard/Enterprise,差異僅在連線字串),**SQL Server Express／LocalDB(EngineEdition=4)已淘汰**——單庫撞其 10GB 上限,首次碰庫即以 `sql_server_express_unsupported` 硬擋;開發／測試環境亦收緊為「非 Express 且 ≥2022」,測試連 jetapp、隔離庫 `JET_Test`。跨 provider 姓名排序以 `COLLATE Latin1_General_BIN2` 對齊(BMP 中文碼位序=SQLite 位元序)。等價測試抽象基底(手算 fixture、命中傳票集合斷言)讓 DuckDB 等未來 provider 只需新增子類即可跑同一套。
