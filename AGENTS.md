...
本檔是「地圖」，不是「規則全文」。每條規則只有一個權威出處（本檔或 `docs/` 的現況規格），其他文件只做指路，不另立第二份事實。動手前只讀與當前任務相關的那一份來源文件；**repository 是唯一事實來源——不要臆測或自創沒有白紙黑字寫下來的規則**。有缺漏就回報並補進對應文件，不要在腦內補完。
...
Read `.claude/skills/minimalist-ui/SKILL.md` only for frontend visual design work. 它是 JET 前端視覺設計的唯一權威,優先於 `frontend-design` 外掛 skill——外掛 skill 是命名空間隔離的通用工具,不會自動讓位給專案 skill,JET 介面一律以 `minimalist-ui` 為準。
...
- Agent 不得自行發動或主動提議 `git commit` / `git push`。版控只在**使用者自行驗證完成果、明確下達指令**之後才執行——使用者不採高頻多輪版控,任務中途的階段性綠燈不是 commit 的理由。
- 子代理開發(SDD)過程同樣**不產生任何 commit**——連暫時性 WIP commit 也不行,維持 `main` 單一線性。需要把某段改動單獨取出做複審時,用**不產生 commit 的 tree 隔離**:`git add -A && git write-tree` 取得 tree SHA,複審包 = `git diff <base-tree-sha> <head-tree-sha>`,事後 `git reset` 還原索引,全程不落任何 commit。
...
- `docs/README.md` 寫作規範第 5 條(文件隨程式碼同 commit 更新)約束的是**變更的打包方式**——同一變更的文件與程式碼不得分屬不同 commit——不是授權 agent 自行發動 commit。
