...
/// <summary>
/// 完整性差異 keyset 分頁(SQLite)。共用 <see cref="ValidationSql.CompletenessDiffCte"/>;
/// 排序鍵 account_code ASC、游標展開布林式 <c>AND account_code &gt; @cursor</c>(首頁省略)、
/// limit 由 <see cref="SqliteDialect"/> 出 <c>LIMIT @pageSize</c>。not_in_tb 旗標 SQLite 用 GetInt64。
/// </summary>
public sealed class SqliteCompletenessDiffPageRepository(JetProjectDatabase database)
