...
/// <summary>
/// SQL Server 連線設定。BaseConnectionString 指向 instance(來源為環境變數 JET_SQLSERVER_CONNECTION,
/// 見 AppCompositionRoot);為 null/空白時代表未設定,選用 sqlServer 的專案會在連線時得到明確錯誤。
...
/// 預設 JET_DEV(Task 8 再由 config 帶入);測試以獨立庫(如 JET_Test)隔離。
/// </summary>
public sealed record SqlServerConnectionOptions(string? BaseConnectionString, string SingleDatabaseName = "JET_DEV");
