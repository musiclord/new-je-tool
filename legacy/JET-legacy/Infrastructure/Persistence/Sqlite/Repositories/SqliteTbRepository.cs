...
{
    /// <summary>
    /// SQLite implementation of <see cref="ITbRepository"/>. Streams raw TB rows
    /// into <c>staging_tb_raw_row</c> in 5,000-row prepared-statement batches
    /// inside a single transaction. Mirrors <see cref="SqliteGlRepository"/>
    /// semantics; only <c>dataset_kind</c> and the staging table differ.
    /// </summary>
    public sealed class SqliteTbRepository : ITbRepository
    {
        ...
        public SqliteTbRepository(DatabaseOptions databaseOptions)
            : this(databaseOptions.SqliteConnectionString, new SqliteSchemaNames())
        {
        }

        public SqliteTbRepository(string connectionString, ISchemaNames names)
        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var rawTable = _names.Resolve(JetTable.StagingTbRawRow);
            ...
            if (!headerSeen)
            {
                throw new InvalidOperationException("TB file has no header row.");
            }
