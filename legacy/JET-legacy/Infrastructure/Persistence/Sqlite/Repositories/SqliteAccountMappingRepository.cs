...
{
    /// <summary>
    /// SQLite implementation of <see cref="IAccountMappingRepository"/>. Streams
    /// raw AccountMapping rows into <c>staging_account_mapping_raw_row</c> in
    /// 5,000-row prepared-statement batches inside a single transaction. Mirrors
    /// <see cref="SqliteGlRepository"/> / <see cref="SqliteTbRepository"/>
    /// semantics; only <c>dataset_kind</c> and the staging table differ.
    /// </summary>
    public sealed class SqliteAccountMappingRepository : IAccountMappingRepository
    {
        private const int BatchSize = 5_000;
        private const string DatasetKind = "accountMapping";

        private readonly string _connectionString;
        private readonly ISchemaNames _names;

        public SqliteAccountMappingRepository(DatabaseOptions databaseOptions)
            : this(databaseOptions.SqliteConnectionString, new SqliteSchemaNames())
        {
        }

        public SqliteAccountMappingRepository(string connectionString, ISchemaNames names)
        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var rawTable = _names.Resolve(JetTable.StagingAccountMappingRawRow);
            ...
            if (!headerSeen)
            {
                throw new InvalidOperationException("AccountMapping file has no header row.");
            }
